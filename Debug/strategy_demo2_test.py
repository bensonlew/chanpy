...
if __name__ == "__main__":
    ...
    data_src = Parquet_API(code, k_type=KL_TYPE.K_5M, begin_date=begin_time, end_date=end_time, autype=AUTYPE.QFQ)  # 初始化数据源类
    is_hold = False
    last_buy_price = None
    for klu in data_src.get_kl_data():  # 获取单根K线
        chan.trigger_load({KL_TYPE.K_5M: [klu]})  # 喂给CChan新增k线
